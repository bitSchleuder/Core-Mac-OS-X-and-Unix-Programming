...
/* compile with 
cc -g -Wmost -o chatterserver chatterserver.m
*/
...
// there's one of these for each connected user
...
typedef struct chatterUser {
    ...
    unsigned int currentMessageSize;	// what the length byte says we should get
    int 	bytesRead;		// what we've read (not including length)
...
// our listening socket appeared in a readfd from select.  That means there's
// a connection there we can accept
...
void acceptConnection (int listenFd)
{
    ...
    result = accept (g_listenFd, (struct sockaddr *)&address, &addressLength);
	...
	(void) write (fd, gripe, strlen(gripe));
	goto bailout;
    }
...
void broadcastMessage (const char *username, const char *message)
{
	...
	if (scan->fd != 0) {
	    ...
	    (void) writev (scan->fd, iovector, 3);
	}
	scan++;
    }
...
void readNickname (chatterUser *user)
{
    ...
    // see if we've read anything yet
	...
	result = read (user->fd, user->buffer + user->bytesRead, readLeft);
	
	if (result == readLeft) {
	    // have the whole nickname
	    memcpy (user->username, user->buffer, user->currentMessageSize);
...
void readMessage (chatterUser *user)
{
	...
	// other side closed
	cleanUpUser (user);
	// ok to skip message sending, since we should have sent all complete
	// messages already
	goto bailout;
    ...
    // now see if we have any complete messages we can send out to other folks
    // the begining of the buffer should have the length byte, plus any
