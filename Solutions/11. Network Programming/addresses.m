...
/* compile with
cc -g -Wmost -o addresses addresses.m
*/
...
int main (int argc, char *argv[])
{
	...
	if (hostinfo == NULL) {
	    fprintf (stderr, "error wtih gethostbyname / apple.  error %s\n",
		     hstrerror(h_errno));
	} else {
	    char **scan;
	    ...
	    printf ("    h_addrtype: %d (%s)\n", 
		    hostinfo->h_addrtype,
		    (hostinfo->h_addrtype == AF_INET) ? "AF_INET" : "unknown");
