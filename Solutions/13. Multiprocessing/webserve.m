...
/* compile with
cc -g -Wmost -o webserve webserve.m
*/
...
void reapChildren ()
{
	...
	if (childPid < 0) {
	    // even though the man page says that we shouldn't get this with
	    // WNOHANG as an option to wait3, it sometimes happens
	    if (errno != ECHILD) {
	    ...
	    fprintf (stderr, "child %ld terminated %s\n",
		     (long)childPid, WIFEXITED(status) ? "normally" : "abnormally");
...
// (like SIGCHLD) or a new connection comes in.  If it's a new connection,
// fork off a child to process the request
...
void acceptRequest (int listenSocket)
{
    ...
    result = accept (listenSocket, (struct sockaddr *)&address, &addressLength);
	...
	if (errno == EINTR) {
		...
		g_childSignaled = 0;
		reapChildren();
		// note that g_childSignaled is cleared before reapChildren is
		// called, in case another sigchld happened during reapChildren,
		// we won't lose it
		goto bailout;
	    }
	} else {
